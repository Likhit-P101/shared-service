...
const levelLogger = () => {
    return env === ENV_TYPE.Prd ? 'info' : 'debug';
};
...
// กำหนดการตั้งค่า การปริ้น Logger
...
const consoleTransport = new Console({
    level: levelLogger(),
    format: combine(label({ label: env }), timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), colorize({ all: true }), printLogger)
});
